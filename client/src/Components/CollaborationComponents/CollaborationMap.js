...
import { StyledDialog } from './StyledCollaboration';
...
function CollaborationMap() {
	...
	// concat null to show the overall stats at the end
	const list = Object.keys(collaborationData).concat(null);
	...
	useEffect(() => {
		...
		const uniqueCountries = Object.values(mapData.features).map((item) => item.properties.ADMIN);
		...
		uniqueCountries.forEach((country) => {
			...
			const countryData = Object
				.values(selectedData)
				.filter((data) => Object.keys(data).includes(country));
				...
				<MapContainer style={{ backgroundColor: colors.map_background, height: '80vh' }} zoom={2} center={[0, 0]}>
					<GeoJSON
						...
						style={(feature) => ({
							...
							weight: 1,
							fillOpacity: scaleOpacity(
								countryData[feature.properties.ADMIN].count,
								maxCollaboration,
							),
						})}
					...
					>
						<DialogTitle
							...
							style={{
								fontSize: '14px', width: '100%', display: 'flex', flexDirection: 'row', justifyContent: 'space-between',
							}}
						>
							{`${selectedYear || 'Cumulative collaborations'} - ${selectedFeature || ''}: 
									...
									<StyledDialog>
										{countryData[selectedFeature].affiliation && (
											<div className="title">
												...
												{' '}
												(
												{countryData[selectedFeature].affiliation.length}
												)
										...
										{countryData[selectedFeature].pmid && (
											<div className="title">
												...
												{' '}
												(
												{countryData[selectedFeature].pmid.length}
												)
												<div className="section">
